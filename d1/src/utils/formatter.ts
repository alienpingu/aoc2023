...
const arrayFrom0to100 = [
    ...
    'ninety', 'ninetyone', 'ninetytwo', 'ninetythree', 'ninetyfour', 'ninetyfive', 'ninetysix', 'ninetyseven', 'ninetyeight', 'ninetynine','zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine',
    'ten']; 

const re = new RegExp(`${arrayFrom0to100.join("|")}`, "g")

export default function formatter(rawCalibration:string):string {
    let cleanResult = rawCalibration;
    const foundList = rawCalibration.match(re);
    if (foundList && foundList.length > 0) {
            foundList.forEach(found => cleanResult = cleanResult.replace(found, replaceList[found]));
    }
    return cleanResult
}